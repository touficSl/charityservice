...
public class UserController {
	...
	public ResponseEntity<?> projectlist(@RequestHeader(name = "Accept-Language", required = false) Locale locale,
		...
		return userService.projectlist(locale, false, page, size, search, sortcolumn, descending, draw, username);
	}
	
	@RequestMapping(value = {"/donate", "/{version}/donate"}, 
			method = RequestMethod.POST, headers = "Accept=application/json") 
	public ResponseEntity<?> donate(
			@RequestHeader(name = "Accept-Language", required = false) Locale locale,
			HttpServletRequest request, 
			@PathVariable(name = "version", required = false) String version,
			@RequestBody DonateRq rq) throws UnsupportedEncodingException { 

		Users user = (Users) request.getAttribute("user");
		return userService.donate(locale, user, rq);
	}
    ...
    public ResponseEntity<?> getCharityStatistics(
		...
		Users user = (Users) request.getAttribute("user");
        return charityStatisticsService.getCharityStatistics(user.getUsername());
    }
}
